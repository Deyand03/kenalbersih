        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-emerald-500">
                <div>
                    ...
                    <h3 class="text-3xl font-black text-emerald-600">{{ $stats['wargaAktif'] }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-amber-400">
                <div>
                    <p class="text-amber-500 text-xs font-bold uppercase tracking-wider mb-1">Menunggu Verifikasi</p>
                    <h3 class="text-3xl font-black text-amber-400">{{ $stats['wargaPending'] }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-red-400">
                <div>
                    <p class="text-red-500 text-xs font-bold uppercase tracking-wider mb-1">Nonaktif / Pindah</p>
                    <h3 class="text-3xl font-black text-red-400">{{ $stats['wargaNonaktif'] }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden">
                ...
                <form action="{{ route('rt.data_warga') }}" method="GET" class="w-full md:w-auto relative">
                    ...
                    @if (request('search'))
                        <a href="{{ route('rt.data_warga') }}"
                            class="absolute right-3 top-3 text-xs text-red-500 hover:underline">Reset</a>
                    @endif
                </form>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100">
                            ...
                            <tr class="group hover:bg-emerald-50/30 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-gradient-to-br {{ $warga->status == 'Aktif' ? 'from-emerald-100 to-emerald-200 text-[#016B61]' : 'from-slate-100 to-slate-200 text-slate-500' }} flex items-center justify-center font-bold text-sm shadow-sm">
                                            {{ substr($warga->nama, 0, 2) }}
                                        </div>
                                        <div>
                                            <p
                                                class="font-bold text-slate-700 {{ $warga->status == 'Nonaktif' ? 'line-through opacity-60' : '' }}">
                                                {{ $warga->nama }}</p>
                                            <p class="text-xs text-slate-400">
                                                {{ $warga->user->email ?? 'Email tidak tersedia' }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @elseif($warga->status == 'Pending')
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-500 border border-amber-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-amber-400"></span> Pending
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-500 border border-red-200">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="relative" x-data="{ open: false }">
                                        <!-- Form Toggle Status -->
                                        <form action="{{ route('rt.data_warga.status', $warga->id) }}" method="POST"
                                            class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            ...
                                            @if ($warga->status == 'Aktif')
                                                <button type="button" onclick="confirmDeactivate(this)"
                                                    class="btn btn-sm btn-error btn-outline gap-2 group-hover:bg-red-500 group-hover:text-white hover:border-red-600 transition-all rounded-lg">
                                            ...
                                            @else
                                                <button type="submit"
                                                    class="btn btn-sm btn-success btn-outline gap-2 group-hover:bg-emerald-500 group-hover:text-white hover:border-emerald-600 transition-all rounded-lg">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-slate-400 text-sm mt-1">Coba kata kunci lain atau belum ada warga
                                            yang terdaftar.</p>
                                    </div>
                                </td>
                            </tr>
